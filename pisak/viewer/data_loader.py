...
#: Time of the database insertion of the last photo, given in seconds
#: since the epoch. Can be used as reference time
PHOTO_REF_TIME = _datetime_to_float(
    database_agent.get_last_photo_insertion_time())


#: Time of the database insertion of the last album, given in seconds
#: since the epoch. Can be used as reference time
ALBUM_REF_TIME = _datetime_to_float(
    database_agent.get_last_album_insertion_time())


#: Time of the last database modification, given in seconds
#: since the epoch. Can be used as reference time
...
def load_new():
    """
    Load previously unloaded photos to the database.
    """
        ...
        if os.path.getmtime(current) > ref_time:
            ...
            for photo_path in [os.path.join(current, name) for name in files]:
                if (os.path.getctime(photo_path) > ref_time and
                        os.path.splitext(photo_path)[-1].lower() in
                        EXTENSIONS):
